...
export default function Tvshow() {
  ...
  async function getTrending(page) {
    let { data } = await axios.get(`https://api.themoviedb.org/3/discover/tv?api_key=d9acecc58f05e7f6676c8b3a4c506126&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=true&page=${page}`)
    setTv(data.results)
        ...
        <div className="col-md-3 " key={index}>
          <Link className=' text-decoration-none text-white' to={`/moviesdetails/${item.id}/${mediaType}`}>
            <div className="content position-relative">
              {item.poster_path ? <img src={'https://image.tmdb.org/t/p/w500' + item.poster_path} className='w-100' alt="tv" /> : <img src={'https://image.tmdb.org/t/p/w500' + item.profile_path} className='w-100' alt="tv" />}
              {item.vote_average ?
